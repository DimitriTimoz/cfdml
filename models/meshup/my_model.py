    ...
    def forward(self, data: Data) -> Tensor:
        ...
        for r in range(self.R): # the 𝑟-th mesh graph
            ...
            if r < self.R - 1:
                ...
                mp_output = self.up_sampling_processors[r](nodes_embedding_up_sampling, up_scale_edge_index, up_scale_edge_embeddings) # Useless computation to fix
                print("after up", torch.isnan(mp_output).sum())
                node_embedding[pf_node_indices_range[0]:pf_node_indices_range[1]] = mp_output[:pf_node_indices_range[1]-pf_node_indices_range[0]]
